...
const Chat = () => {
  ...
  return (
    <ScrollView
      ...
      onContentSizeChange={() => chatContainerRef.current.scrollToEnd()}
      style={styles.chat}
    ...
    >
      {state.history.map((message, index) => {
        ...
        if (
          ...
          state.history[index + 1].from === message.from
        ) {
        ...
        if (message.type === 'txt') {
          ...
          const icon =
            playbackObjectId === message.time && isPlayingAudio === true
              ? pauseIcon
              : playIcon;
          output = (
            <View key={message.time} style={style}>
              <TouchableOpacity
                ...
                onPress={() =>
                  voiceMsgBtnHandler(message.msg.uri, message.time)
                }
              >
                <Image source={icon} />
              </TouchableOpacity>
              <Text style={{ fontSize: 22 }}>
                {parseTime(Math.round(message.msg.duration / 1000))}
              </Text>
            </View>
          );
        }
